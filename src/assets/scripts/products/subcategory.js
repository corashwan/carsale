...
config(function($stateProvider) {
		$stateProvider
			.state('root.subcategory', {
				url: 'category/:id/:sub_id',
				views: {
					'container@': {
						controller: 'SubCategoryDetailCtrl',
						templateUrl: 'scripts/products/subcategory.html',
					}
				}
			});
	})
	.controller('SubCategoryDetailCtrl', function($scope, $timeout, $localStorage, WISHLIST, $stateParams, $state, $filter, $q, $rootScope, API, DATA, PagerService, gettextCatalog, NOTIFIER, LANGUAGE) {
		...
		if(!$scope.subcategory){
			NOTIFIER.error(gettextCatalog.getString('404 Not Found'), gettextCatalog.getString("Subategory was not found!"))
			$state.go('root.home')
		}

		$scope.slider = {
			  min: $scope.subcategory.price_range.min,
			  max: $scope.subcategory.price_range.max,
			  options: {
			    floor: $scope.subcategory.price_range.min,
			    ceil: $scope.subcategory.price_range.max
			  }
			};
		$scope.categories = DATA.get('_categories')
		$rootScope.page_title = $filter('localize')($scope.subcategory, 'name')
	...
	$scope.redirect_subcategory = function(subcategory, sub ){
		$state.go('root.sub_subcategory', ({id:$scope.category.slug, sub_id:subcategory.slug, sub_sub_id:sub.slug}))
	}
	...
	$scope.$watch(function(){
	  return $localStorage['country'];
		...
		if(newCodes != oldCodes){
				var df = $q.defer();
				var promises = [];
				promises.push(API.getAll('/ads?page_size=10000&featured=true&country='+$localStorage['country']+'&subcategory='+$scope.subcategory.id, '_featured_category_products'));
				promises.push(API.getAll('/ads?country='+$localStorage['country']+'&subcategory='+$scope.subcategory.id, '_category_products'));
		...
		$scope.$watch("filter_gov", function(b, a){
			...
			var promises = [];
			if($scope.filter_gov){
				promises.push(API.getAll('/ads?governorate='+$scope.filter_gov+'&country='+$localStorage['country']+'&subcategory='+$scope.subcategory.id, '_category_products'));
			} else {
				promises.push(API.getAll('/ads?country='+$localStorage['country']+'&subcategory='+$scope.subcategory.id, '_category_products'));
			}
			$q.all(promises).then(function(){
