...
config(function($stateProvider) {
		$stateProvider
			.state('root.category_product_detail', {
				url: 'category/:id/ads/:product_id',
				views: {
					'container@': {
						controller: 'CategoryProductDetailCtrl',
						templateUrl: 'scripts/products/category_product.html',
					}
				}
			});
	})
	.controller('CategoryProductDetailCtrl', function($scope, $filter, LANGUAGE, $timeout, WISHLIST, $localStorage, $sce, USER, $stateParams, $state, $q, $rootScope, ERRORTIPS, API, DATA, gettextCatalog, NOTIFIER) {
		...
		$q.all(promises).then(function(){
			...
			$scope.product.share_url = $state.href('root.category_product_detail', {id:$scope.product.category.slug,
